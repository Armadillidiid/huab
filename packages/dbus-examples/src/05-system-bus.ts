...
async function introspectService(serviceName: string, objectPath: string) {
  ...
  try {
    ...
    const introspectable = obj.getInterface(
      "org.freedesktop.DBus.Introspectable",
    ) as unknown as IntrospectableInterface;
...
async function queryNetworkManager() {
  ...
  try {
    ...
    const obj = await bus.getProxyObject(serviceName, objectPath);
    const nm = obj.getInterface(
      "org.freedesktop.NetworkManager",
    ) as unknown as NetworkManagerInterface;
...
async function queryLoginManager() {
  ...
  try {
    ...
    const obj = await bus.getProxyObject(serviceName, objectPath);
    const login = obj.getInterface(
      "org.freedesktop.login1.Manager",
    ) as unknown as LoginManagerInterface;
  ...
  try {
    ...
    if (devices.length > 0) {
      ...
      if (firstDevice) {
        ...
        const device = deviceObj.getInterface(
          "org.freedesktop.UPower.Device",
        ) as unknown as UPowerDeviceInterface;
...
async function sendDesktopNotification() {
  ...
  try {
    ...
    const obj = await bus.getProxyObject(serviceName, objectPath);
    const notifications = obj.getInterface(
...
async function monitorSystemEvents() {
  ...
  try {
    ...
    const login = obj.getInterface(
      "org.freedesktop.login1.Manager",
    ) as unknown as LoginManagerEventsInterface;
...
async function main() {
  ...
  if (args.includes("--introspect")) {
    await introspectService("org.freedesktop.NetworkManager", "/org/freedesktop/NetworkManager");
    return;
  }
